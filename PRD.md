...
- 200px min width and 80px height
...
- The bottom part holds a basic button component aligned middle center, and is 120px high and anchored to the bottom, it has a background of 10% black
...
filter component
- 100% width within the filter container (500px with padding applied as above etc) and responsive height based on its contents
- It has a 20px soft shadow #000000 at 15% and 00xy
- is split into a header/body
- header height is dictated by the title copy
- title copy is Inter Semi Bold at 20px in white and has 30px padding L/R and 10px padding T/B, and #FFFFFF when Clinical trial is selected and #14365B (Zoryve Midnight Blue) when Practice based is
- header background is #14365B when Clinical trial is selected and #FFB81C when Practice based is
- radius corners only on top corners of 16px
- body part has same radius corners but only on the bottom and a background of #689ACF 35% opacity
- content in the body has 30px padding all around typical apart from the condition filter which is 20px

Condition button component
- 100% width of its container
- 52px height
- all content left middle aligned
- 13px text size and this is the same for all of the filter component options, and regular weight when not selected and bold when selected, text is 20px apart from the dot and this is the same for all filter options with their various other styles of selection
- 8px dot to the left inset with equal padding/margin to look good, #FFFFFF when selected #14365B and 40% alpha when not
- no background shown when not selected

Radio button style multiple selection
- unselected state 30px w/h 6px radius corners and 2px solid stroke #FFFFFF line
- selected state has a #FFFFFF fill and a small tick inside using zoryve black #1F2B2B, and a 10px glow using zoryve yellow #FDE939
- text like other options is 20px away
...
- because of the background trying to look seamlessly attached to the filter options, this one has an override on the left margin/padding, so instead of 30px like all the others its 120px on the left on this one
...
- footer bottom
-- full width 80px height #408BAB (Zoryve Blue) Background
-- two buttons to the left using these SVGS
--- public/UI/footer-info.svg
--- public/UI/footer-refs.svg
-- logo to the right middle vertical alignment and 30px padding to the right - public/UI/logo-zoryve.svg
- important safety information - ISI Panel above it
-- we will put this there as a placeholder for now and populate later, for now it has a 200px height and full width using midnight blue for background
-- has a + button floating a little out of the box on the y axis which allows the user to open and read this panel - public/UI/plus-button.svg, it is 20px up over hanging the bounds of the ISI panel background and 30px in from the right, and has a 6px solid midnight blue border
-- when pressed to open the panel the button rotates 45% which a smooth animation to become a close button
